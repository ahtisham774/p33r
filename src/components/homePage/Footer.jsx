  ...
  const socialLinks = [
    {
      ...
      link: '#',
       color: 'text-[#2148A5]',
      bg: 'hover:bg-[#2148A5] hover:text-white'
    },
    {
      svg: (
        <svg
      ...
      link: '#',
       color: 'text-[#2E6FE8]',
      bg: 'hover:bg-[#2E6FE8] hover:text-white'
    },
    {
      svg: (
        <svg
      ...
      link: '#',
      color: 'text-[#E82E2E]',
      bg: 'hover:bg-[#E82E2E] hover:text-white'
    },
    {
      svg: (
        <svg
      ...
      link: '#',
      color: 'text-[#2EBAE8]',
      bg: 'hover:bg-[#2EBAE8] hover:text-white'
    }
  ]
  ...
  return (
    <footer className='py-4  w-full'>
      <div className='w-full'>
        <div className='flex flex-wrap w-full justify-end gap-8 items-center mb-8'>
       
            <nav className='flex flex-wrap justify-between gap-8'>
              {links.map((link, index) => (
                <Link
                  key={index}
                  href={link.href}
                  className='text-[#171A1F] text-lg font-normal hover:text-gray-900 transition-colors'
                >
                  {link.text}
                </Link>
              ))}
        ...
        <div className='flex flex-wrap gap-5 justify-center items-center pt-4 border-t'>
          <select className='rounded-xl bg-[#F6F7F8] p-1.5 outline-none border px-4 '>
            <option value='en'>English</option>
            <option value='es'>Spanish</option>
          </select>
          <div className='text-base flex-1 flex-wrap-reverse flex items-center gap-8 justify-center font-normal text-[#424955]'>
            <p>© {new Date().getFullYear()} P33R, Inc.</p>
            <ul className='flex gap-6 list-disc'>
            <li>
              <a
                href='#'
                className='text-base font-normal  hover:text-gray-900'
              >
                Terms
              </a>
            </li>
          ...
          {socialLinks.map((socialLink, index) => (
            <a
              ...
              key={index}
              className={`bg-[#F6F7F8] border border-[#EAECF0] size-8  flex items-center justify-center shrink-0 rounded-full ${socialLink.color} ${socialLink.bg} `}
            >
              {socialLink.svg}
            </a>
          ))}
